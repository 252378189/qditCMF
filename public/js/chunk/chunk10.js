...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(233)("14a3d46e", content, false, {});
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
...
/* harmony default export */ __webpack_exports__["default"] = ({
    ...
    methods: {
        ...
        handleTools: function handleTools(type, index, row) {
            ...
            if (type == 'recovery') {
                Object(__WEBPACK_IMPORTED_MODULE_2__api_article__["i" /* recovery_article */])(row.id).then(function (response) {
                    //成功响应动态移除表格项
                    _this.handleDeleteRow(index);
                    //提示信息
                ...
                //删除数据
                Object(__WEBPACK_IMPORTED_MODULE_2__api_article__["j" /* recovery_del */])(row.id).then(function (response) {
                    //成功响应动态移除表格项
                    _this.handleDeleteRow(index);
                    //提示信息
                    _this.$message.success(response.data.msg);
